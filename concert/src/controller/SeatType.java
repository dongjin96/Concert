...
public class SeatType extends Concert{
	...
	void book(String[][] s,String[][] a,String[][] b){ // 좌석을 예약을 할 때 사용하는 메소드
		...
		case 1: // S석을 선택
			while(true)
			{
				...
				{
					if( seatcheck(seatT,seatN,name) == 1) // seat 클래스에 있는 seatChk() 메소드를 이용하여 이미 예약된 자리인지를 확인
						break;
				}
	...
	void cancel(String[][] s, String[][] a, String[][] b){ // 취소를 할 때 사용하는 메소드
		...
		if(cancel(num, name) == 0) // 취소할 이름이 예약자 목록에 없을경우
